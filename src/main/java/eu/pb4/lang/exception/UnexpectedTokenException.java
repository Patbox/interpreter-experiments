...
public class UnexpectedTokenException extends Exception implements ScriptConsumer {
    ...
    public String getMessage() {
        if (this.input == null) {
            ...
            return "Invalid token '" + this.input.substring(tokenFound.start(), Math.min(tokenFound.end() + 1, this.input.length())) + "' at line " + val[0] + " position " + val[1] + " (\"" + GenUtils.getSubStringWithoutNewLines(this.input, tokenFound.start() - 10, tokenFound.end() + 10) + "\")! Required: "
                    + this.tokenTypeRequired + " Found: " + this.tokenFound.type();
        }
    }
}
